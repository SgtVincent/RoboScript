...
class RGBDCameraSet:
    ...
    '''
    def __init__(self, cameras_list: List[str], namespace="", buffer_size=10, use_aligned_depth=True, sub_pcl=True) -> None:
        
        self.ns = namespace
        self.buffer_size = buffer_size
        self.cameras_list = cameras_list
        self.sub_pcl = sub_pcl
        self.cameras: Dict[str, RGBDCamera] = {}
        for camera_name in cameras_list:
            self.cameras[camera_name] = RGBDCamera(camera_name, namespace=namespace, 
...
if __name__ == "__main__":
    ...
    rospy.init_node("gazebo_cameras", anonymous=False, log_level=rospy.DEBUG)
    save_dir = "/home/junting/repo/llm-manipulation-bench/grasp_detection/data"

    rgbd_camera_set = RGBDCameraSet(
        cameras_list=['camera_left', 'camera_right', 'camera_top'],
    ...
    for i in range(num_data):
        rospy.sleep(1.0)
        data = rgbd_camera_set.get_latest_data()
        
        # save rgb images to file
        for camera_name, rgb_image in zip(data['camera_names'], data['rgb_image_list']):
            # write rgb_8 image to file
            rgb_image_path = os.path.join(save_dir, camera_name, f"rgb_{i}.png")
            cv2.imwrite(rgb_image_path, rgb_image)
            rospy.loginfo(f"{camera_name}: rgb image saved to {rgb_image_path}")
