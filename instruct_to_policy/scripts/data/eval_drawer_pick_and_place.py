...
def run_eval(evaluator: Evaluator, object_name, object_file_path, object_pose, drawer_idx, 
             pull_distance=0.2, repeat_times=10):
    """
    ...
    for i in range(repeat_times):
        ...
        try:
            ...
            # open the drawer
            open_the_drawer(evaluator.env, drawer_idx=0, distance=pull_distance)
...
if __name__ == '__main__':
    ...
    log_file = rospy.get_param('~log_file', f'eval_drawer_pick_and_place.log')
    log_file_path = os.path.join(pkg_root, 'log', log_file)
    # make log directory if not exist
    os.makedirs(os.path.dirname(log_file_path), exist_ok=True)
    
    # results file 
    result_file_path = os.path.join(pkg_root, f'data/benchmark/eval_results/drawer_pick_and_place_{time_str}.json')
    ...
    for object_id in metadata['objects'].keys():
        ...
        run_eval(evaluator, object_name, object_file_path, object_pose, drawer_idx=0, pull_distance=args.pull_distance, repeat_times=10)
