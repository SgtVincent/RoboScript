  ...
  void GazeboRosMoveItPlanningScene::UpdateCB()
  {
         ...
         ++model_it)
    {
           ...
           ++link_it)
      {
        ...
        // Skip this link if does not have any collisions
...
#endif
        // Moveit planning scene new api requires ColllisionObject.pose to be set
        object.pose.position.x = link_pose.Pos().X();
        object.pose.position.y = link_pose.Pos().Y();
        object.pose.position.z = link_pose.Pos().Z();
        object.pose.orientation.x = link_pose.Rot().X();
        object.pose.orientation.y = link_pose.Rot().Y();
        object.pose.orientation.z = link_pose.Rot().Z();
        object.pose.orientation.w = link_pose.Rot().W();
        // ROS_DEBUG_STREAM_NAMED("GazeboRosMoveItPlanningScene",model_name << " (link): " <<object.pose);
